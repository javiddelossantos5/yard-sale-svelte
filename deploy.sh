...
# Step 2: Create/update .env.production file
echo -e "${BLUE}[2/7] Creating/updating .env.production file...${NC}"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}  VITE_API_BASE_URL=${API_BASE_URL}${NC}"
else
    echo -e "${RED}✗ Failed to create .env.production${NC}"
    exit 1
fi
echo ""
...
echo -e "${BLUE}[4/7] Stopping old container (if exists)...${NC}"
...
echo -e "${BLUE}[6/7] Starting new container...${NC}"
if docker run -d \
    --name ${CONTAINER_NAME} \
    -p ${PORT}:3000 \
...
echo -e "${BLUE}[7/7] Verifying container...${NC}"
sleep 2

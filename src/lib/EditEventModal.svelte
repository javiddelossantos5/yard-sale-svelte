<script lang="ts">
	import { updateEvent, type EventUpdate, type Event, getAuthenticatedImageUrl } from './api';
	...
	let formData = $state<EventUpdate>({
		type: 'event',
		title: '',
		description: '',
		status: 'upcoming',
	...
	$effect(() => {
		if (isOpen && event) {
			...
			formData = {
				...
				employment_type: event.employment_type || undefined,
				...
				gallery_urls: event.gallery_urls ? [...event.gallery_urls] : [],
				featured_image: event.featured_image || ''
			};
			showOptionalFields = true; // Show optional fields when editing
		}
	});

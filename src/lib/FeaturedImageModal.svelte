<script lang="ts">
	import {
		getYardSaleImages,
		setYardSaleFeaturedImage,
		removeYardSaleFeaturedImage,
		getAuthenticatedImageUrl,
		type YardSaleImagesResponse
	} from '$lib/api';
	...
	let images = $state<YardSaleImagesResponse | null>(null);
	...
	async function loadImages() {
		...
		error = null;
		try {
			images = await getYardSaleImages(yardSaleId);
		...
		try {
			...
			if (selectedType === 'photo') {
				...
				if (index !== -1) {
					...
					if (originalIndex !== -1) {
						request.photo_index = originalIndex;
					}
				} else {
			...
			const result = await setYardSaleFeaturedImage(yardSaleId, request);
	...
	async function removeFeaturedImage() {
		...
		try {
			await removeYardSaleFeaturedImage(yardSaleId);
			selectedImage = null;
			selectedType = null;
			onSuccess?.();
			onClose();
			...
			<div class="overflow-y-auto p-6" style="max-height: calc(90vh - 140px)">
				...
				{:else if images}
					<div class="space-y-6">
						...
						<!-- Photos from Yard Sale -->
						{#if images.photos && images.photos.length > 0}
							<div>
								<h3 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">
									Yard Sale Photos ({images.photos.length})
								</h3>
								<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
						...
						{#if (!images.photos || images.photos.length === 0) && (!images.uploaded_images || images.uploaded_images.length === 0) && !images.featured_image}
							<div class="py-12 text-center">
								<p class="text-sm text-gray-500 dark:text-gray-400">
									No images available. Add photos to your yard sale first.
								</p>
							</div>
						{/if}
					</div>

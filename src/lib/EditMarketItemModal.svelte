<script lang="ts">
	...
	// Initialize form data from item when opened
	$effect(() => {
		if (isOpen && item) {
			formData = {
				name: item.name || '',
	...
	async function handleSubmit(event: Event) {
		...
		try {
			...
			const payload: Partial<MarketItemCreate> = {
				...formData,
				photos,
							...
							<div>
								...
								<ImageUpload
									...
									onImagesChange={(images) => {
										...
										// Keep current featured_image if it's still in the list, otherwise use first image or keep existing
										if (!formData.featured_image || !images.includes(formData.featured_image)) {
											formData.featured_image = images[0] || formData.featured_image || '';
										}
									}}
								/>
